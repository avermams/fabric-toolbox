        ...
        foreach ($wsId in $workspaceIds) {
            ...
            do {
                ...
                while ($true) {
                    ...
                    try {
                        ...
                        if ($error_returned -ne 429) {
                            Write-Warning "Get Recoverable Items API failed with error code ($error_returned)."
                            $skipWorkspace = $true
                            break
                        }
                        ...
                        Write-Host "Throttled, waiting $retryAfterSeconds seconds before retrying to get recoverable workspace items."
                ...
                if ($skipWorkspace -eq $true) {
                    Write-Warning "Skipping workspace $wsId due to failure in retrieving its soft-deleted content."
                    break
                }
